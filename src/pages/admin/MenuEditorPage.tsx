...
export default function MenuEditorPage() {
  ...
  const regenerateMutation = useMutation({
    mutationFn: () => updateMenu(menuId, { name: menu!.name, description: menu!.description, is_active: menu!.is_active }),
  ...
  const updateCatDisplayMutation = useMutation({
    mutationFn: ({ catId, payload }: { catId: number; payload: Partial<Category> }) =>
  ...
  function handleDisplayChange(catId: number, field: 'price_display' | 'is_full_width', value: string | boolean) {
    // Optimistic update
    qc.setQueryData<Category[]>(['menu-categories', menuId], old =>
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-xl font-display font-semibold" style={{ color: 'var(--coffee)' }}>{menu.name}</h1>
            {menu.description && (
              <p className="text-sm mt-0.5" style={{ color: 'var(--muted)' }}>{menu.description}</p>
            )}
            <span className={`mt-1 inline-block px-2 py-0.5 text-xs rounded-full font-medium ${menu.is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
              {menu.is_active ? 'Activo' : 'Inactivo'}
            </span>
          </div>
          <div className="flex gap-2">
            <button
              onClick={() => { setForm({ name: menu.name, description: menu.description ?? '', is_active: menu.is_active }); setEditModal(true) }}
            ...
            {menuCategories?.map((cat) => {
              ...
              return (
                <SortableCategoryBlock
                  ...
                  onAddProdChange={(catId, prodId) => setAddProdIds({ ...addProdIds, [catId]: prodId })}
                  onAddProd={(catId) => {
                    const prodId = addProdIds[catId]
                    if (prodId) {
                      addProdMutation.mutate({ prodId, pos: (menuProducts?.length ?? 0) + 1 })
  ...
  return (
    <>
      ...
      <div className="hidden md:flex gap-0" style={{ minHeight: 'calc(100vh - 160px)' }}>
        <div
      ...
      <div className="md:hidden">
        ...
        >
          <button
            ...
            style={{
              backgroundColor: tab === 'editor' ? 'var(--gold)' : 'var(--card-bg)',
          ...
          <button
            ...
            className="flex-1 py-2 text-sm font-medium transition-colors cursor-pointer"
            style={{
              backgroundColor: tab === 'preview' ? 'var(--gold)' : 'var(--card-bg)',
        ...
        {tab === 'editor' ? editorContent : (
          <div className="rounded-lg" style={{ backgroundColor: 'var(--bg-alt)', border: '1px solid var(--border)' }}>
            {previewContent}
          </div>
        )}
      </div>
